...
/* ----------- */
const SIGNATURE_TIMESTAMP_REGEX = /signatureTimestamp:(\d+)/g, SHIM = Platform_1.Platform.getShim();
...
function getDownloadURL(format, decipherFunction, nTransformFunction) {
    ...
    const decipher = (url) => {
        const SEARCH_PARAMS = new URLSearchParams(url), PARAMS_URL = SEARCH_PARAMS.get('url')?.toString() || '';
