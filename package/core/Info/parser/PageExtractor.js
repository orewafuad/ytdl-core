...
class YouTubePageExtractor {
    static getWatchHtmlUrl(id, options) {
        return `${Urls_1.default.getWatchPageUrl(id)}&hl=${options.lang || 'en'}&bpctr=${Math.ceil(Date.now() / 1000)}&has_verified=1`;
    }
    ...
    static getEmbedPageBody(id, options) {
        const EMBED_PAGE_URL = `${Urls_1.default.getEmbedUrl(id)}?hl=${options.lang || 'en'}`;
